			...
			<div className="flex">
				{React.Children.map(children, (child, index) => {
					if (React.isValidElement<IBaseTabProps>(child)) {
						...
						return (
							<button
								className={`uppercase px-4 py-2 ${index === 0 && 'rounded-tr-md'} ${
									index === React.Children.toArray(children).length - 1 && 'rounded-tl-md'
								} ${
									index === activeTab ? 'bg-teal-500 text-white' : 'bg-neutral-100 text-neutral-600'
								}`}
