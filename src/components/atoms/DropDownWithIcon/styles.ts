...
export const baseDropDownStyles = cva('flex justify-between items-center rounded-lg px-2', {
	variants: {
		intent: {
			default:
			...
			error: 'text-red-600 border border-2 border-red-600 placeholder-red-400',
		},
		fullWidth: {
			true: 'w-full',
		},
...
export const optionSelectStyles = cva(
	'absolute block bg-neutral-100 rounded text-right mt-2 border-2 border-teal-500 max-h-28 overflow-y-auto z-10',
	{
		variants: {
			...
			size: {
				xs: `w-32 ${SIZE.TYPOGRAPHY.BODY4}`,
				sm: `w-[12rem] text-md${SIZE.TYPOGRAPHY.BODY3}`,
				md: `w-[20rem] text-lg ${SIZE.TYPOGRAPHY.BODY3}`,
				lg: `w-[40rem] text-xl ${SIZE.TYPOGRAPHY.BODY2}`,
				xl: `w-[50rem] text-xl ${SIZE.TYPOGRAPHY.BODY2}`,
			},
		},
		defaultVariants: {
			isShow: false,
