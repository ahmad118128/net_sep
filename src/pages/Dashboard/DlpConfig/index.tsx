...
export function DlpConfig() {
	...
	const { data, error, isLoading, mutate } = useSWR<IResponsePagination<IFileType>>(
		endpoint,
		http.fetcherSWR
	);
		...
		<div className="w-full p-4">
			...
			{isLoading ? (
				...
				listWhiteList.map((item) => (
					<FileTypeCard key={item.id} fileType={item} onClickActions={handleOnClickActions} />
				))
			) : (
				<NoResult />
			)}
			...
			<Modal
				...
				type="success"
				content={
					<UpdateFileTypeModal handleClose={handleCloseUpdateModal} fileType={activeFileType} />
				}
			/>
		</div>
	);
